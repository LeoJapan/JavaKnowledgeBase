* [Web3 基础入门]()
    ...
    * [四、以太坊(ETH)是什么?](#%E5%9B%9B%E4%BB%A5%E5%A4%AA%E5%9D%8Aeth%E6%98%AF%E4%BB%80%E4%B9%88)
     ...
     * [十、使用ERC-721 构建您自己的NFT]()
       * [10.1 ERC-20是什么?](#91-erc-20%E6%98%AF%E4%BB%80%E4%B9%88)  
# Web3 基础入门
...
#### 9.3 编写代码
使用[Remix IDE](https://remix.ethereum.org/)创建一个智能合约文件，此处任意命名，我的命名为<kbd><b>LeoJapanToken.sol</b></kbd>,代码如下：
...
contract LeoJapanToken is ERC20 {
...
pragma solidity ^0.8.0;
```
> 此行指定要使用的 Solidity 编译器版本。 <kbd><b>^0.8.0</b></kbd> 表示大于 <kbd><b>0.8.0</b></kbd> 的任何版本。 通常，您会希望使用最新的 Solidity 编译器版本，因为新版本通常意味着新功能或优化。
```Solidity
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC20/ERC20.sol";
```
> 该行从 [OpenZeppelin](https://www.openzeppelin.com/) (OZ) 导入 <kbd><b>ERC-20</b></kbd> 令牌标准。 OZ 是一家以太坊安全公司。 除此之外，OZ 还为流行的智能合约标准开发参考合约，这些标准经过全面测试且安全。 每当实施需要遵守标准的智能合约时，尽量找到 OZ 参考实施，而不是从头开始重写整个标准。  
>   
> 如果需要，您可以通过以下链接查看 <kbd><b>ERC-20</b></kbd> 标准合约的实施 - https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC20/ERC20.sol
```Solidity
contract LeoJapanToken is ERC20 {
    ...
}
```
> 这行是我们的 Solidity 文件中指定了一个名为 LeoJapanToken 的新合约。 另外，它说这个合约是 <kbd><b>ERC-20</b></kbd> 的一个实例。 本例中的 <kbd><b>ERC-20</b></kbd> 指的是我们从 OpenZeppelin 导入的标准合约。  
>  
> 本质上，我们正在扩展从 OpenZeppelin 导入的 <kbd><b>ERC-20</b></kbd> 标准合约。 因此，<kbd><b>ERC-20</b></kbd> 内置的所有功能和逻辑都可供我们使用，我们可以在其上添加自己的自定义逻辑。
>
> 如果您熟悉面向对象编程的原则，您可以将其视为扩展另一个类的类。
...
```
> 本质上，我们创建了在首次部署智能合约时调用的<kbd><b>构造函数</b></kbd>。 在构造函数中，我们需要来自用户的两个参数 - <kbd><b>_name</b></kbd> 和 <kbd><b>_symbol</b></kbd>，它们指定我们的加密货币的名称和符号。 例如。 名称 = Ethereum，符号 = ETH。
>
> 之后发生的事情更有趣。 在指定构造函数后，我们立即调用 <kbd><b>ERC20(_name, _symbol)</b></kbd>。
>
> 我们从 OpenZeppelin 导入的 <kbd><b>ERC-20</b></kbd> 合约有自己的构造函数，需要 <kbd><b>name</b></kbd> 和 <kbd><b>symbol</b></kbd> 参数。 由于我们正在扩展 ERC20 合约，因此在部署我们的合约时需要初始化 ERC20 合约。 因此，作为构造函数的一部分，我们还需要在 <kbd><b>ERC-20</b></kbd> 合约上调用构造函数。  
>
> 因此，我们为我们的合约提供 <kbd><b>_name</b></kbd> 和 <kbd><b>_symbol</b></kbd> 变量，我们立即将其传递给 <kbd><b>ERC-20</b></kbd> 构造函数，从而初始化 <kbd><b>ERC-20</b></kbd> 智能合约。
```Solidity
_mint(msg.sender, 10 * 10 ** 18);
```
> <kbd><b>_mint</b></kbd>是<kbd><b>ERC-20</b></kbd>标准合约<kbd><b>内部的</b></kbd>一个函数，也就是说它只能被合约本身调用。 外部用户无法调用此函数。
>  
> 由于您作为开发人员希望在部署此合约时收到一些Token，因此我们调用 <kbd><b>_mint</b></kbd> 函数来为 <kbd><b>msg.sender</b></kbd> 铸造一些Token。
> 
>  <kbd><b>_mint</b></kbd> 有两个参数——要<kbd><b>铸造的地址</b></kbd>和要<kbd><b>铸造的代币数量</b></kbd>
>
> <kbd><b>msg.sender</b></kbd> 是以太坊虚拟机注入的全局变量，它是进行此交易的地址。 由于您将是部署此合同的人，因此您的地址将在 <kbd><b>msg.sender</b></kbd> 中。
> 
> <kbd><b>10 * 10 ** 18</b></kbd> 指定您希望将 10 个完整令牌铸造到您的地址。  

**注意：您可能想知道为什么我们不只在数量中写 10，而不是 10 ** 18（实际上是 10 ^ 18）。**

> 本质上，Solidity 不支持浮点数——即小数。 此外，由于 ERC20 代币与金钱打交道，因此使用浮点数不是一个好主意。
>
> 例如，考虑使用支持浮点数的语言进行的简单计算 <kbd><b>(1/3) * 3</b></kbd>。 你认为这会返回什么？
>
> 如果您认为它会返回 1，那您就错了。
>
> 由于浮点计算的不准确性，由于计算机不能表示无限数量的数字，<kbd><b>(1/3) * 3</b></kbd> 实际上产生类似 0.999999999 的结果。
>
> 因此，在表示金融货币时，由于计算错误，不使用小数。 作为替代方案，我们将每种货币表示为相对于该货币的最小不可分割部分的金额。 例如，1 美元表示为 100 美分，因为在处理美元时，您不能小于 1 美分。 在那个编号系统中，1 美分只是 1，而不是 0.01。 $0.33 表示为 33，而不是 (1/3)。
>
> <kbd><b>ERC20</b></kbd> 代币默认使用 18 位小数。 所以在这种情况下，1 个完整的 <kbd><b>LeoJapanToken</b></kbd> 实际上表示为 <kbd><b>10 ^ 18</b></kbd>。因此，要获得 10 个完整的 <kbd><b>LeoJapanToken</b></kbd>，我们使用 <kbd><b>10 * 10 ** 18</b></kbd>。
>
> **10 ** 18(10 ^ 18)表示10的18次方，1后面18个0**
#### 9.4 编译
> 通过按“保存”（在 Windows 上按 CTRL + S，在 Mac 上按 Command + S）或转到 Remix 中的<kbd><b>Compiler</b></kbd>选项卡，选择 <kbd><b>LeoJapanToken.sol</b></kbd>，然后点击<kbd><b>Compile</b></kbd>来编译您的合约。

![LeoJapanToken01901](01_Freshman/pic/01-9-01.png)
#### 9.5 部署
> 转到 Remix 中的 <kbd><b>Deployer</b></kbd> 选项卡。
> 
> 选择 <kbd><b>Injected Provider - MetaMask</b></kbd> 环境（确保您在 Goerli 测试网络上），并连接您的 Metamask 钱包。

选择 <kbd><b>LeoJapanToken.sol</b></kbd> 合约，并为构造函数参数 <kbd><b>_name</b></kbd> 和 <kbd><b>_symbol</b></kbd> 输入值。

![LeoJapanToken01902](01_Freshman/pic/01-9-02.png)

> 单击<kbd><b>Transact</b></kbd>并批准来自 Metamask 的交易以部署您的合约！

![LeoJapanToken01903](01_Freshman/pic/01-9-03.png)
> 部署后，合同应显示在<kbd><b>Deployed Contracts</b></kbd>部分下。 点击<kbd><b>Copy Address</b></kbd>按钮复制合约地址。

![LeoJapanToken01904](01_Freshman/pic/01-9-04.png)
> 去 Goerli Etherscan 搜索你的合约地址，你应该在那里看到它！
>
> 截取它的屏幕截图并在 Discord 上分享以展示您新创建的令牌

![LeoJapanToken01905](01_Freshman/pic/01-9-05.png)
#### 9.6 在 Metamask 中查看代币
> 您可能会注意到，即使您为您的地址铸造了代币，它们也不会出现在 Metamask 中。
>
> 这是因为 Metamask 无法检测到随机的 ERC20 代币余额（因为它们实际上有数十万个）。 他们有一个可以自动显示的最著名的 ERC20 代币列表，但除此之外，对于您自己的代币，您通常需要告诉 Metamask 手动将其添加到您的钱包。
> 
> 为此需要做以下步骤：
> * 复制你的合约地址
> * 打开 Metamask 并在“资产”选项卡中单击“导入令牌”
> * ![LeoJapanToken01906](01_Freshman/pic/01-9-06.png)
> * 输入您的代币合约地址，它应该会自动检测名称和小数位数 
> * ![LeoJapanToken01907](01_Freshman/pic/01-9-07.png)
> 
> * 点击添加，您将在 Metamask 中看到您的余额！
> * ![LeoJapanToken01908](01_Freshman/pic/01-9-08.png)
> * **到此为止已经成功部署并铸造了您自己的 ERC20 代币**
> * ![LeoJapanToken01910](01_Freshman/pic/01-9-10.png)


**Deployed Contracts - LeoJapanToken(自己的加密货币)**
> 0x8aC4FE54490464ca599a624C11d0dF50F08565BE